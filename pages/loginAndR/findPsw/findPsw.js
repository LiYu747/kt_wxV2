...
Page({
    ...
    user.findPaw({
      data: {
            tel:this.data.locadata[0].value,
						sms_code:this.data.locadata[3].value,
						new_secret: this.data.locadata[1].value,
						new_secret_confirmation: this.data.locadata[2].value,
      },
      fail: () => {
        wx.hideLoading()
        wx.showToast({
  ...
  Onblur(e) {
    ...
    if(index == 1){
      let msg = 'locadata.[1].errorMessage'
       if(value.length<6){
        this.setData({
          [msg]: '密码至少6位'
        })
    ...
    if (index == 2) {
      if (value == this.data.locadata[1].value) {
